...
- Four independently inherited template fields may appear in root or folder config notes: `template-file-prefix`, `template-file-date`, `template-file-path`, and `template-file-applied-To`. Nearest-folder values win per field. Defaults use `{{date}}_`, `DD.MM.YYYY` with a time suffix on same-date collisions, `templates/example.md`, and `md`; `*` applies to all extensions.
