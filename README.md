...
Folder templates have separate `template-md`, `template-canvas`, and `template-base` mappings in root or folder config notes. Each mapping contains one vault-relative template path whose value is `[date format, filename prefix, apply filename convention]`, for example `template-md: { example.md: [DD.MM.YYYY, "{{date}}_", true] }`. A missing folder mapping inherits the nearest parent mapping and then the vault default; an empty mapping disables that type. The Markdown default copies `example.md` from the global `template-folder` (`templates` by default), applies the date prefix, and adds a 24-hour time suffix on same-date collisions. Canvas and Base default to disabled. Turning filename treatment off still copies the selected template without renaming the new file.
