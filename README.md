...
Folder templates have separate `template-md`, `template-canvas`, and `template-base` mappings in root or folder config notes. Each mapping contains one vault-relative template path whose value is `[date format, filename prefix, apply filename convention]`, for example `template-md: { example.md: [DD.MM.YYYY, "{{date}}_", true] }`. A missing folder mapping inherits the nearest parent mapping and then the vault default; an empty mapping disables that type. The Markdown default copies `example.md` from the global `template-folder` (`templates` by default), applies the date prefix, and adds a 24-hour time suffix on same-date collisions. Canvas and Base default to disabled. Turning filename treatment off still copies the selected template without renaming the new file. In settings, each file type has one switch that opens or closes its four template fields. A disabled global switch stores an empty mapping. A disabled folder override inherits its nearest rule; an enabled override with an empty template file explicitly disables that type for the selected folder.

Bare global template filenames resolve inside `template-folder`; global paths containing folders are exact vault-relative paths. Folder override paths resolve under that same template folder by default, including their subfolders. This prevents template-only first-level folders from appearing as books when the template folder is excluded. The override can instead use exact vault-relative paths. Saving overrides creates missing parent folders and empty valid Markdown, Canvas, or Base template files, and never replaces an existing template. Invalid Windows filename characters in generated prefixes are replaced with hyphens. Legacy Markdown template fields are read during migration and rewritten in the new format when their settings are saved.
